...
<xsl:stylesheet version="1.0"
    ...
    <xsl:param name="timestamp_from"/>
    <xsl:param name="timestamp_until"/>
    <xsl:output
    method="html"
...
-->
  ...
  <xsl:template match="res:uri">
    ...
    <!--<xsl:text>&lt;</xsl:text>-->
	<a href="https://charme-test.cems.rl.ac.uk/sparql?query={$query}&amp;output=xml&amp;stylesheet=http://localhost:8080/cmsaf/css/xml-to-html-links.xsl">
    <xsl:value-of select="."/>
    </a>
    <!--<xsl:text>&gt;</xsl:text>-->
  </xsl:template>
  ...
  <xsl:template match="res:sparql">
	...
	<style>
      ...
      <body>
		...
		<h1>CHARME Monitor: Overview</h1>

    <h2>Annotations added/modified between  <xsl:value-of select="$timestamp_from"/> and <xsl:value-of select="$timestamp_until"/> </h2>
